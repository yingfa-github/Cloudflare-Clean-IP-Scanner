...
> _Share my other open source projects:[**TrackersList.com**- List of popular BT Trackers on the entire network! Effectively improve BT download speed~](https://github.com/Ptechgithub/TrackersListCollection)<img src="https://img.shields.io/github/stars/Ptechgithub/TrackersListCollection.svg?style=flat-square&label=Star&color=4285dd&logo=github" height="16px" />_  
> _[**UserScript**- 🐵 Github high-speed download, Zhihu enhancement, automatic seamless page turning, eye protection mode and more than a dozen**Grease Monkey Script**~](https://github.com/Ptechgithub/UserScript)<img src="https://img.shields.io/github/stars/Ptechgithub/UserScript.svg?style=flat-square&label=Star&color=4285dd&logo=github" height="16px" />_  
> _[**SNIProxy**- 🧷 Simple SNI Proxy for your own use (supports all platforms, all systems, front-end agents, simple configuration, etc. ~](https://github.com/Ptechgithub/SNIProxy)<img src="https://img.shields.io/github/stars/Ptechgithub/SNIProxy.svg?style=flat-square&label=Star&color=4285dd&logo=github" height="16px" />_

> This project also supports**Other CDN/Website IP**Delayed speed test (such as:[CloudFront](https://github.com/Ptechgithub/CloudflareScanner/discussions/304)、[Gcore](https://github.com/Ptechgithub/CloudflareScanner/discussions/303)CDN), but you need to find the address yourself when testing the download speed.

> for**Proxy Suite Cloudflare CDN**users, please note that this should be**back up plan**, rather than**only solution**, do not rely too much on[#382](https://github.com/Ptechgithub/CloudflareScanner/discussions/382)[#383](https://github.com/Ptechgithub/CloudflareScanner/discussions/383)
...
2.  Double click to run`CloudflareST.exe`File (Windows system), waiting for the speed test to complete...
...
> If flat**Very low latency**(such as 0.xx), then CloudflareST**The agent left during the speed test**, please close the proxy software before testing the speed.  
...
> _exist**cell phone**A simple tutorial to run the CloudflareST speed test independently on:**[Android](https://github.com/Ptechgithub/CloudflareScanner/discussions/61)、[Android APP](https://github.com/xianshenglu/cloudflare-ip-tester-app)、[IOS](https://github.com/Ptechgithub/CloudflareScanner/discussions/321)**_
...
> People who are new to CloudflareST may be confused**There are obviously 30 IPs available for delay speed testing, so why are there only 3 left in the end?**  
> What does the queue in the download speed test mean? Do I still have to queue up when I download the speed test?

CloudflareST will first delay the speed test, during which the number of available IPs will be displayed in real time on the right side of the progress bar (`可用: 30`), but note that the available quantity refers to**Number of IPs that passed the test without timeout**, has nothing to do with the upper and lower limits of delay and packet loss conditions. When the delayed speed test is completed, because it is also specified**Delay upper and lower limits, packet loss**conditions, so after filtering according to the conditions, only`10`Already (that is, waiting for the download speed test)`队列：10`）。
...
> **hint**：Windows**PowerShell**Just put in the following command`CloudflareST.exe`Change to`.\CloudflareST.exe`That’s it.  
> **hint**: Linux system only needs to change the following command`CloudflareST.exe`Change to`./CloudflareST`That’s it.

* * *

#### #CMD runs CloudflareST with parameters
...
Many people open CMD to**absolute path**Running CloudflareST will report an error. This is because the default`-f ip.txt`The parameter is a relative path, and you need to specify the absolute path of ip.txt. However, this is too troublesome, so it is recommended to enter the CloudflareST program directory to**relative path**Run as:

**method one**：

1.  Open the directory where the CloudflareST program is located
...
4.  Enter a command with parameters, such as:`CloudflareST.exe -tll 50 -tl 200`Ready to run
...
3.  Enter a command with parameters, such as:`CloudflareST.exe -tll 50 -tl 200`Ready to run

> Of course, you can also open a CMD window at will and enter such as`cd /d "D:\Program Files\CloudflareST"`to enter the program directory

> **hint**: If using**PowerShell**Just put in the command`CloudflareST.exe`Change to`.\CloudflareST.exe`That’s it.

</details>

* * *

#### #Windows shortcut to run CloudflareST with parameters
...
Right click`CloudflareST.exe`document -**\[Create Shortcut]**, then right-click the shortcut -**\[Attributes]**, modify its**Target**：
...
-   _<https://github.com/xianshenglu/cloudflare-ip-tester-app>_  
    _**CloudflareST Android APP[#202](https://github.com/Ptechgithub/CloudflareScanner/discussions/320)**_

-   _<https://github.com/mingxiaoyu/luci-app-CloudflareScanner>_  
    _**CloudflareST OpenWrt router plug-in version[#174](https://github.com/Ptechgithub/CloudflareScanner/discussions/319)**_

-   _<https://github.com/immortalwrt-collections/openwrt-cdnspeedtest>_  
    _**CloudflareST OpenWrt native compiled version[#64](https://github.com/Ptechgithub/CloudflareScanner/discussions/64)**_

-   _<https://github.com/hoseinnikkhah/CloudflareScanner-English>_  
    _**English language version of CloudflareST (Text language differences only)[#64](https://github.com/Ptechgithub/CloudflareScanner/issues/68)**_

> _Here we only collect some of the CloudflareST related derivative projects that have been promoted in this project. If there are any omissions, please let me know~_
